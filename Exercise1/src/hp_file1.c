...
int HP_CreateFile(char *fileName){
	...
	BF_OpenFile(fileName , &file_desc);   // περνω το file_desc για το αρχειο που δημιουργω απο την συναρτηση BF_openfile
...
HP_info* HP_OpenFile(char *fileName, int *file_desc){
	...
	if (hpInfo == NULL) 
	{
		...
		return NULL;        //επιστροφη NULL σε περιπτωση error
	}
	...
	if(file == NULL)
	{
		...
		BF_GetBlockCounter(file_desc , &blocks_in_file);
		printf("\n number of block in this file are : %d\n" , blocks_in_file);
...
int HP_InsertEntry(int file_desc,HP_info* hp_info, Record record){
	...
	{  
		...
		if (available_data_size >= sizeof(Record))//σε αυτη την περιπτωση εχει χωρο για τουλαχιστον μια ακομη εγγραφη
		{      
...
int HP_GetAllEntries(int file_desc,HP_info* hp_info, int value){    
 ...
 //δημιουργια του μπλοκ
	...
	for (int j = 1; j <= blocks_in_file; j++){
		...
		data=(char*)data + sizeof(HP_block_info);     //offset για να παμε στις εγγραφες του μπλοκ
		...
		for (int i = 0; i < hpblockinfo->records; i++)
		{
			...
			{        
				...
				//printf("The record above is at the block : %d\n\n" , j);
				block_read=j;     //εχουμε διαβασει οσα μπλοκ οσα εχουμε τρεξει στην επαναληψη
			}
			data=(char*)data + sizeof(Record);      //offset για να παμε στην επομενη εγγραφη 
		}
		if (hpblockinfo->next_block== NULL)
		{
			if(block_read==-1)      //αν δεν εγινε σωστα η επαναληψη, εκτυπωσε σφαλμα
				printf("\nThe record was not found\n");
			BF_UnpinBlock(block);   
			return block_read ;     //επεστρεψε τα μπλοκ που διαβαστηκαν ή -1 σε περιπτωση σφαλματος
		}
